        ...
        <div class="container">
            @if($cart->quantity > 0)
                <h1 class="cart-header">
                    {{translate('Hooray,_you’ve_successfully_added')}} {{$cart->quantity}} {{translate('item_to_your_cart.')}}
                </h1>
                ...
                <div class="row cart-items-holder">
                            ...
                            <div class="cart-item">
                                <div class="row cart-item-details">
                                    <div class="cart-item-remove">
                                        <a href="{{route('cart.item.remove',$item->removeParameters())}}">
                                            <i class="fa fa-times"></i>
                                        </a>
                                    </div>
                                    <div class="cart-item-price">
                                        <sub>
                                            <span>Total:</span> {{ payment()->currency_symbol }}{{sprintf('%.2f',$item->total)}}
                                        </sub>
                                        <sub>
                                            @if($item->deposit>0)
                                                <span>Deposit Due:</span> {{ payment()->currency_symbol }}{{sprintf('%.2f',$item->deposit)}}
                                            @else
                                    ...
                                    <div class="col-md-2">
                                        <div class="item-card-img">
                                            <img src="{{asset('images/items/thumbSm/'.$model->img)}}"
                                                 class="aspect-img" alt="{{$model->title}}">
                                        </div>
                                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <span class="cart-header-total">
                            ...
                            <label>{{ payment()->currency_symbol }}{{sprintf('%.2f',$cart->total)}}</label>
                        </span>
                            <span class="cart-header-total">
                            Total Deposit:
                            <label>{{ payment()->currency_symbol }}{{sprintf('%.2f',$cart->deposit)}}</label>
                        </span>
